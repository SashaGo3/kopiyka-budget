...
# Everything else is passed through to capture.sh (--phone / --watch / --theme / --only) or,
# for --frame runs, to frame.mjs (e.g. --contact-sheet).
#
# Output: screenshots/appstore/iphone-6.9/*.png, iphone-6.5/*.png, watch/*.png (+ contact-sheet.png).
# Uses its own simulators ("Kopiyka Shots" / "Kopiyka Shots Watch"); yours are never touched.
...
if has install; then
  ...
  log "Installing on \"$PHONE_NAME\" and \"$WATCH_NAME\""
