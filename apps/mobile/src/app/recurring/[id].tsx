...
import { REMINDER_OPTIONS, getReminderDaysBefore } from "@/lib/settings";
...
export default function RecurringEdit() {
  ...
  const keys = useMemo(() => ({ cat: newPickKey("rcat"), acc: newPickKey("racc"), date: newPickKey("rdate"), time: newPickKey("rtime"), payee: newPickKey("rpayee"), amount: newPickKey("ramount"), repeat: newPickKey("rrep"), unit: newPickKey("runit"), count: newPickKey("rcount"), remind: newPickKey("rrem"), post: newPickKey("rpost"), tags: newPickKey("rtags"), tx: newPickKey("rtx") }), []);
  ...
  const commit = async () => {
    ...
    mutate((d) => {
      const base = { account_id: account.id, amount_minor: amountMinor * (kind === "expense" ? -1 : 1), category_id: categoryId, payee: payee.trim() || null, tag_ids: JSON.stringify(tagIds), frequency: freq, interval, start_date: existing?.start_date ?? start, next_date: start, notify: daysBefore !== null ? 1 : 0, notify_days_before: daysBefore ?? 1, auto_post: autoPost ? 1 : 0, time_of_day: time } as const;
    ...
    <View style={{ flex: 1, backgroundColor: C.bgGrouped }}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: S.md }} keyboardShouldPersistTaps="handled" automaticallyAdjustKeyboardInsets>
        ...
        <Card style={{ marginTop: S.md }}>
          ...
          <Row icon="bolt" iconColor="#FFD60A" title="Posting" subtitle={autoPost ? "Automatically on the day" : "Ask me first"} onPress={() => option(keys.post, "Posting", POSTING, autoPost ? "auto" : "manual")} style={styles.divider} />
        </Card>
